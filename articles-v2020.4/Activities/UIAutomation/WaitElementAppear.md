...
> **注意：**
>
> **”超时“** 属性的设置需要根据情况去设置，否则默认是项目的超时时间（30 秒）。例：假如我们匹配超时设置成 5000（毫秒），超时设置成 20000（毫秒），执行的时候，我们会在5000（毫秒）循环等待目标元素出现，此时，
>
> - 如果目标元素还未出现，并在剩下的15000（毫秒）中继续等待目标元素出现。
> - 如果目标元素已出现，也需要等到设置的“超时”时间（20000 毫秒）时，才会继续往下执行。

![等待元素出现超时属性](https://docimages.blob.core.chinacloudapi.cn/images/Activities/waitelementappear20211028.png)
