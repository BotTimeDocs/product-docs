...
25. 调试时，在变量面板支持查看值的详细信息。
26. 新建项目时支持选择桌面录制技术 UIA/UIA3。
27. 支持删除命名空间。
28. 支持设置运行时是否最小化编辑器主界面。
29. 支持更多的快捷键，参见[键盘快捷键](Studio/quickStart/KeyboardShortcuts.md)。
30. 支持360安全浏览器。
...
1. 在**设置**页面中新增[关于页](Robot/Settings/About.md)。
...
2. 整体架构升级，上线云端版和私有化版，支持功能包括：
   - **[调度队列](Console/queue/aboutqueue.md)**：通过机器人集群实现对流程任务动态分配。
   - **[任务记录](Console/job/aboutJob.md)**：展示所有流程任务的执行记录及日志详情。
