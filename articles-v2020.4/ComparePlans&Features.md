# 《个人版VS企业版--产品介绍》

<br><br>根据你的需求弹性选择配置，[登录注册](https://auth.encoo.com/Account/Register?ReturnUrl=%2fconnect%2fauthorize%2fcallback%3fclient_id%3dencoo_console_v3%26redirect_uri%3dhttps%253A%252F%252Fconsole.encoo.com%252Fcallback%26response_type%3dcode%26scope%3dopenid%2520profile%2520apigateway%2520offline_access%26state%3d43a4f32cc2fe4b5f906c10e18f5b8d4d%26code_challenge%3dtYhQskRW1PrPByc8_6HdVkUKQ7H8ZnDvmDLXE6QBBIY%26code_challenge_method%3dS256%26response_mode%3dquery)后可直接下载个人版产品<br>

<br>个人版分免费版和专业版，满足不同人群的使用需求，让更多人享受RPA带来的便捷与高效<br><br>

<img width = '400'  src ="https://doria-encooacademyimages.oss-cn-shanghai.aliyuncs.com/2022/12/28/16722140831651.jpg"/>



<br><br><br>


<table>
<thead>
  <tr>
    <th colspan="2" rowspan="4"></th>
    <th></th>
    <th>个人免费版</th>
    <th>个人专业版</th>
    <th>企业SAAS版</th>
    <th>企业私有定制版</th>
  </tr>
  <tr>
    <th>规格</th>
    <th>免费</th>
    <th>付费</th>
    <th rowspan="3"><a href="https://www.encoo.com/contactus" target="_blank" rel="noopener noreferrer">联系商务</a></th>
    <th rowspan="3"><a href="https://www.encoo.com/contactus" target="_blank" rel="noopener noreferrer">联系商务</a></th>
  </tr>
  <tr>
    <th>原价</th>
    <th>0</th>
    <th>联系我们</th>
  </tr>
  <tr>
    <th>折扣价</th>
    <th>0</th>
    <th>联系我们</th>
  </tr>
</thead>
<tbody>
  <tr>
    <td rowspan="17">编辑器</td>
    <td>系统环境</td>
    <td>系统环境</td>
    <td>Windows</td>
    <td>Windows</td>
    <td>Windows、麒麟、统信等</td>
    <td rowspan="78">个人版和企业版功能全部支持；<br>支持产品功能定制；<br>私有化部署；<br>VIP专属在线和现场技术支持等。<br><br>详情请<a href="https://www.encoo.com/contactus" target="_blank" rel="noopener noreferrer">联系我们</a></td>
  </tr>
  <tr>
    <td rowspan="8">内置组件库</td>
    <td>常用组件</td>
    <td>支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td>AI组件</td>
    <td>支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td>控制台组件</td>
    <td>支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td>SAP</td>
    <td>不支持</td>
    <td>不支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td>CV组件</td>
    <td>不支持</td>
    <td>不支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td>手机自动化</td>
    <td>不支持</td>
    <td>不支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td>远程桌面</td>
    <td>不支持</td>
    <td>不支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td>桌面控件专有</td>
    <td>不支持</td>
    <td>不支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td>运行/调试</td>
    <td>运行/调试</td>
    <td>支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td>自定义组件</td>
    <td>自定义组件</td>
    <td>支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td rowspan="4">市场</td>
    <td>组件市场</td>
    <td>支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td>代码市场</td>
    <td>支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td>流程模板市场</td>
    <td>支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td>元素市场</td>
    <td>支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td>Git版本管理</td>
    <td>Git版本管理</td>
    <td>支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td>智能录制</td>
    <td>智能录制</td>
    <td>支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td rowspan="16">机器人</td>
    <td>系统环境</td>
    <td>系统环境</td>
    <td>Windows</td>
    <td>Windows</td>
    <td>Windows、麒麟、统信等</td>
  </tr>
  <tr>
    <td rowspan="2">流程库</td>
    <td>本地执行</td>
    <td>支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td>控制台调度</td>
    <td>不支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td>定时任务</td>
    <td>定时任务</td>
    <td>不支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td>待执行计划</td>
    <td>待执行计划</td>
    <td>不支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td rowspan="2">任务记录</td>
    <td>本地管理</td>
    <td>不支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td>上传到控制台并管理</td>
    <td>不支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr>
  <tr>
     <td rowspan="6">流程执行</td>
    <td>基本</td>
    <td>支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td>录制视频</td>
    <td>不支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td>执行过程截图</td>
    <td>不支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr>
 <tr>
    <td>以管理员权限运行</td>
    <td>支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr> 
  <tr>
    <td>独立桌面</td>
    <td>不支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td>超时设置</td>
    <td>不支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td>本地激活</td>
    <td>本地激活</td>
    <td>不支持</td>
    <td>不支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td rowspan="2">设置</td>
    <td>自动解锁</td>
    <td>不支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr>
 <tr>
    <td>PDR会话保持</td>
    <td>不支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr> 
<tr>
    <td rowspan="38">控制台</td>
    <td rowspan="5">仪表盘</td>
    <td>总体监控</td>
    <td>支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr>
<tr>
    <td>机器人监控</td>
    <td>不支持</td>
    <td>支持</td>
    <td>支持</td>
</tr> 
<tr>
    <td>机器人组监控</td>
    <td>不支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr> 
<tr>
    <td>机器人运行统计</td>
    <td>不支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr> 
<tr>
    <td>用户流程统计</td>
    <td>不支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr> 
 <tr>
    <td rowspan="3">自动化卓越中心</td>
    <td>需求发起</td>
    <td>不支持</td>
    <td>不支持</td>
    <td>支持</td>
  </tr>
<tr>
    <td>需求跟进</td>
    <td>不支持</td>
    <td>不支持</td>
    <td>支持</td>
  </tr> 
<tr>
    <td>数据分析</td>
    <td>不支持</td>
    <td>不支持</td>
    <td>支持</td>
</tr> 
<tr>
    <td rowspan="9">RPA中心</td>
    <td>流程包管理</td>
    <td>不支持</td>
    <td>数量：限额</td>
    <td>数量：不限</td>
  </tr>
<tr>
    <td>机器人管理</td>
    <td>不支持</td>
    <td>数量：不限</td>
    <td>数量：不限</td>
  </tr>
<tr>
    <td>机器人组管理</td>
    <td>不支持</td>
    <td>数量：限额</td>
    <td>数量：不限</td>
  </tr>
<tr>
    <td>流程部署管理</td>
    <td>不支持</td>
    <td>数量：限额</td>
    <td>数量：不限</td>
  </tr>
<tr>
    <td>流程编排</td>
    <td>不支持</td>
    <td>数量：限额</td>
    <td>数量：不限</td>
  </tr>
<tr>
    <td>任务队列</td>
    <td>不支持</td>
    <td>数量：限额</td>
    <td>数量：不限</td>
  </tr>
<tr>
    <td>执行记录</td>
    <td>不支持</td>
    <td>数量：不限</td>
    <td>数量：不限</td>
  </tr>
<tr>
    <td>计算机管理</td>
    <td>不支持</td>
    <td>数量：不限</td>
    <td>数量：不限</td>
  </tr>
<tr>
    <td>日历管理</td>
    <td>不支持</td>
    <td>数量：限额</td>
    <td>数量：不限</td>
  </tr>
<tr>
    <td rowspan="2">ViCode</td>
    <td>未发布应用</td>
    <td>数量：限额</td>
    <td>数量：限额</td>
    <td>数量：不限</td>
  </tr>
<tr>
    <td>发布应用</td>
    <td>数量：0</td>
    <td>数量：少许（可另外购买）</td>
    <td>数量：不限</td>
  </tr>
<tr>
    <td rowspan="3">AI中心</td>
    <td>文档理解发布模板</td>
    <td>数量：少许</td>
    <td>数量：限额</td>
    <td>数量：不限</td>
  </tr>
<tr>
    <td>OCR服务</td>
    <td>每个云扩AI服务免费500次</td>
    <td>每个云扩AI服务免费2000次</td>
    <td>每个云扩AI服务免费2000次</td>
  </tr>
 <tr>
    <td>AI服务管理</td>
    <td>支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr>
<tr>
    <td rowspan="4">智能知识库</td>
    <td>知识库</td>
    <td>数量：少许</td>
    <td>数量：限额</td>
    <td>数量：不限</td>
  </tr>
<tr>
    <td>应用</td>
    <td>数量：少许</td>
    <td>数量：限额</td>
    <td>数量：不限</td>
  </tr>
<tr>
    <td>额度</td>
    <td>免费调用次数总额度：若干</td>
    <td>免费调用次数总额度：若干</td>
    <td>免费调用次数总额度：若干</td>
  </tr>
 <tr>
    <td>管理</td>
    <td>支持</td>
    <td>支持</td>
    <td>支持</td>
  </tr>
<tr>
    <td rowspan="5">数据中心</td>
    <td>资产管理</td>
    <td>不支持</td>
    <td>数量：若干</td>
    <td>数量：不限</td>
  </tr>
<tr>
    <td>数据表</td>
    <td>不支持</td>
    <td>数量：若干</td>
    <td>数量：不限</td>
  </tr>
<tr>
    <td>文件服务</td>
    <td>不支持</td>
    <td>数量：若干</td>
    <td>数量：300</td>
  </tr>
<tr>
    <td>连接管理</td>
    <td>不支持</td>
    <td>数量：限额</td>
    <td>数量：不限</td>
  </tr>
<tr>
    <td>数据队列</td>
    <td>不支持</td>
    <td>数量：限额</td>
    <td>数量：不限</td>
  </tr>
<tr>
    <td rowspan="6">管理中心</td>
    <td>组织结构管理</td>
    <td>不支持</td>
    <td>不支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td>角色管理</td>
    <td>不支持</td>
    <td>不支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td>用户管理</td>
    <td>不支持</td>
    <td>不支持</td>
    <td>支持</td>
  </tr>
<tr>
    <td>审计日志</td>
    <td>不支持</td>
    <td>不支持</td>
    <td>支持</td>
  </tr>
<tr>
    <td>许可证管理</td>
    <td>编辑器：少许<br>机器人：少许</td>
    <td>编辑器：少许<br>机器人：不限</td>
    <td>不限</td>
  </tr>
<tr>
    <td>系统设置</td>
    <td>不支持</td>
    <td>不支持</td>
    <td>支持</td>
  </tr>
<tr>
    <td>权限管理</td>
    <td>权限管理</td>
    <td>不支持</td>
    <td>不支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td rowspan="7">其他</td>
    <td rowspan="2">技术支持</td>
    <td>在线支持</td>
    <td>社区大群</td>
    <td>VIP在线技术支持</td>
    <td>VIP专属技术支持</td>
  </tr>
  <tr>
    <td>现场技术支持</td>
    <td>不支持</td>
    <td>不支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td>产品定制</td>
    <td>产品定制</td>
    <td>不支持</td>
    <td>不支持</td>
    <td>不支持</td>
  </tr>
  <tr>
    <td>私有化部署</td>
    <td>私有化部署</td>
    <td>不支持</td>
    <td>不支持</td>
    <td>不支持</td>
  </tr>
  <tr>
    <td>API集成</td>
    <td>API集成</td>
    <td>不支持</td>
    <td>不支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td>企微集成</td>
    <td>企微集成</td>
    <td>不支持</td>
    <td>不支持</td>
    <td>支持</td>
  </tr>
  <tr>
    <td>钉钉集成</td>
    <td>钉钉集成</td>
    <td>不支持</td>
    <td>不支持</td>
    <td>不支持</td>
  </tr>
</tbody>
</table>
<!-- <table class="css-w46srl-ProductTable">
          <thead>
            <tr>
              <th></th>
              <th></th>
              <th></th>
              <th>个人免费版</th>
              <th>个人收费版</th>
              <th>企业SASS版</th>
            </tr>
          </thead>
          <tbody class="css-11b6sxa-ProductTable">
            <tr>
              <td rowspan="17" class="css-iy3jd3-ProductTable">编辑器</td>
              <td rowspan="1">系统环境</td>
              <td>系统环境</td>
              <td>Windows</td>
              <td>Windows</td>
              <td>Windows、麒麟、统信、欧拉等</td>
            </tr>
            <tr>
              <td rowspan="8">内置组件库</td>
              <td>常用组件</td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td>AI组件</td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td>控制台组件</td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td>SAP</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td>CV组件</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td>手机自动化</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td>远程桌面</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td>桌面专有控制</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td rowspan="1">运行/调试</td>
              <td>运行/调试</td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td rowspan="1">自定义组件</td>
              <td>自定义组件</td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td rowspan="4">市场</td>
              <td>组件市场</td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td>代码市场</td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td>流程模板市场</td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td>元素市场</td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td rowspan="1">Git版本管理</td>
              <td>Git版本管理</td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td rowspan="1">智能录制</td>
              <td>智能录制</td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td rowspan="16" class="css-iy3jd3-ProductTable">机器人</td>
              <td rowspan="1">系统环境</td>
              <td>系统环境</td>
              <td>Windows</td>
              <td>Windows</td>
              <td>Windows、麒麟、统信、欧拉等</td>
            </tr>
            <tr>
              <td rowspan="2">流程库</td>
              <td>本地执行</td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td>控制台调度</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td rowspan="1">定时任务</td>
              <td>定时任务</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td rowspan="1">待执行计划</td>
              <td>待执行计划</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td rowspan="2">任务记录</td>
              <td>本地管理</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td>上传到控制台管理</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td rowspan="6">流程执行</td>
              <td>基本</td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td>录制视频</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td>执行过程截图</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td>以管理员权限运行</td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td>独立桌面</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td>超时设置</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td rowspan="1">本地激活</td>
              <td>本地激活使用</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td rowspan="2">设置</td>
              <td>自动解锁</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td>RDP会话保持</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td rowspan="38" class="css-iy3jd3-ProductTable">控制台</td>
              <td rowspan="5">仪表盘</td>
              <td>总体监控</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td>机器人监控</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td>机器人组监控</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td>机器人运行统计</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td>用户流程统计</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td rowspan="3">自动化卓越中心</td>
              <td>需求发起</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td>需求跟进</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td>数据分析</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td rowspan="9">RPA中心</td>
              <td>流程包管理</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>数量：限额</td>
              <td>数量：不限</td>
            </tr>
            <tr>
              <td>机器人管理</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>数量：不限</td>
              <td>数量：不限</td>
            </tr>
            <tr>
              <td>机器人组管理</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>数量：限额</td>
              <td>数量：不限</td>
            </tr>
            <tr>
              <td>流程部署管理</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>数量：限额</td>
              <td>数量：不限</td>
            </tr>
            <tr>
              <td>流程编排</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>数量：限额</td>
              <td>数量：不限</td>
            </tr>
            <tr>
              <td>任务队列</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>数量：限额</td>
              <td>数量：不限</td>
            </tr>
            <tr>
              <td>执行记录</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>数量：不限</td>
              <td>数量：不限</td>
            </tr>
            <tr>
              <td>计算机管理</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>数量：不限</td>
              <td>数量：不限</td>
            </tr>
            <tr>
              <td>日历管理</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>数量：限额</td>
              <td>数量：不限</td>
            </tr>
            <tr>
              <td rowspan="2">ViCode</td>
              <td>未发布应用</td>
              <td>数量：限额</td>
              <td>数量：限额</td>
              <td>数量：不限</td>
            </tr>
            <tr>
              <td>发布应用</td>
              <td>数量：0</td>
              <td>数量：少许 （可额外购买）</td>
              <td>数量：不限</td>
            </tr>
            <tr>
              <td rowspan="3">AI中心</td>
              <td>文档理解已发布模板</td>
              <td>数量：少许 （免费额度500）</td>
              <td>数量：限额（免费调用次数总额度500）</td>
              <td>数量：不限（免费额度500）</td>
            </tr>
            <tr>
              <td>OCR服务</td>
              <td>每个云扩AI服务免费：500次</td>
              <td>每个云扩AI服务免费：2000次</td>
              <td>每个云扩AI服务免费：2000次</td>
            </tr>
            <tr>
              <td>AI服务管理</td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td rowspan="4">智能知识库</td>
              <td>知识库</td>
              <td>数量：少许</td>
              <td>数量：限额</td>
              <td>数量：不限</td>
            </tr>
            <tr>
              <td>应用</td>
              <td>数量：少许</td>
              <td>数量：限额</td>
              <td>数量：不限</td>
            </tr>
            <tr>
              <td>额度</td>
              <td>免费调用次数总额度：若干</td>
              <td>免费调用次数总额度：若干</td>
              <td>免费总额度：若干</td>
            </tr>
            <tr>
              <td>管理</td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td rowspan="5">数据中心</td>
              <td>资产管理</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>数量：若干</td>
              <td>数量：不限</td>
            </tr>
            <tr>
              <td>数据表</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>数量：若干</td>
              <td>数量：不限</td>
            </tr>
            <tr>
              <td>文件服务</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>数量：若干（小于等于10M）</td>
              <td>数量：300（小于等于100M）</td>
            </tr>
            <tr>
              <td>连接管理</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>数量：限额</td>
              <td>数量：不限</td>
            </tr>
            <tr>
              <td>数据队列</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>数量：限额</td>
              <td>数量：不限</td>
            </tr>
            <tr>
              <td rowspan="6">管理中心</td>
              <td>组织结构管理</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td>角色管理</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td>用户管理</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td>审计日志</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td>许可证管理</td>
              <td>编辑器： 少许 机器人：少许</td>
              <td>编辑器： 少许 机器人：不限</td>
              <td>不限</td>
            </tr>
            <tr>
              <td>系统设置</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td rowspan="1">权限管理</td>
              <td>权限管理</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td rowspan="7" class="css-iy3jd3-ProductTable">其他</td>
              <td rowspan="2">技术支持</td>
              <td>在线支持</td>
              <td>社区大群</td>
              <td>VIP在线技术支持</td>
              <td>VIP专属技术支持</td>
            </tr>
            <tr>
              <td>现场技术支持</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td rowspan="1">产品定制</td>
              <td>产品定制</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td><img alt="" src="./icon_error.svg" /></td>
            </tr>
            <tr>
              <td rowspan="1">私有化部署</td>
              <td>私有化部署</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td><img alt="" src="./icon_error.svg" /></td>
            </tr>
            <tr>
              <td rowspan="1">API集成</td>
              <td>API集成</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td rowspan="1">企微集成</td>
              <td>企微集成</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td>
                <img alt="" src="./icon_success.svg" />
              </td>
            </tr>
            <tr>
              <td rowspan="1">钉钉集成</td>
              <td>钉钉集成</td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td><img alt="" src="./icon_error.svg" /></td>
              <td><img alt="" src="./icon_error.svg" /></td>
            </tr>
          </tbody>
        </table> -->