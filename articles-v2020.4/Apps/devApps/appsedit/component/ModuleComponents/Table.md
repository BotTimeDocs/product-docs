...
| 参数       | 说明 |
| ---------- | ---- |
| 选择表     |默认为空，下拉选择该小程序应用中的任一数据表。      |
| 展示列     |默认选择表的前6列，可通过下拉列表勾选/反选需要展示的列。      |
| 创建新增页 |默认不创建，勾选后创建新增页。<br> 创建组件时，将新建一个页面，加入该表的表单组件，新页面命名为：XX新增页（XX为关联的表名）；组件内“新增”按钮关联该新增页，点击“新增”，将跳转到对应的新增表单页。      |
| 创建详情页 |默认不创建，勾选后创建详情页。<br> 创建组件时，将新建一个页面，加入该表的描述列表组件，新页面命名为：XX详情页（XX为关联的表名）；新建详情页默认不显示选择器操作列“详情”按钮关联该详情页，点击操作列“详情”，将跳转到对应的描述列表页，展示该行详情信息。     |
| 创建编辑页 |默认不创建，勾选后创建编辑页。<br> 创建组件时，将新建一个页面，加入该表的表单组件，新页面命名为：XX编辑页（XX为关联的表名）；操作列“编辑”按钮关联该编辑页，点击操作列“编辑”，将跳转到对应的表单页，展示该行编辑页。|
