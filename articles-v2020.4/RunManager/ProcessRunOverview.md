...
* 名为 Encoo Robot Service 的 Windows 服务程序  
  - 是机器人的核心程序，负责处理机器人的逻辑
  - 一个计算机只存在最多一个该进程
* 名为 Robot.exe 的 Windows 桌面应用程序
  - 是与用户交互的界面程序
  - 计算机中的每个用户都可以启动一个进程，不会互相影响
...
* 系统日志
  - 由机器人自身产生的日志，并且与流程内的逻辑无关，任何流程都会产生该类日志。如：“正在加载流程”、“组件开始：赋值”、“组件错误：未找到指定元素”等等。
  - 这类日志会存放在.log文件中
* 业务日志
  - 由用户通过“写入日志”组件产生的日志，日志内容没有限制，完全由编写流程的人根据需要使用。
  - 这类日志会存放在*-businesslog.log中

### 日志级别
* Debug
  - 调试日志
  - 一般用于流程开发者辅助开发或调试错误
  - 用来记录细致的流程执行信息，如执行步骤。或是将一些变量的值打印出来用于调试流程。
* Info
  - 重要日志
  -  一般用于流程使用者
  -  用来记录需要呈现给流程使用者的一些粗略但关键的信息。
* Error
  -  错误日志
  -  在流程出现异常情况时，用于记录当下的异常信息
  -  错误日志出现时，机器人会相应地抓取当前计算机画面存放于日志目录中，供用户进行错误分析
