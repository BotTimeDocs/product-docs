...
<table><td bgcolor=	#F0F8FF>当指定一个桌面元素时。 **点击方式** 属性选择 **设置控件**后，仅支持**左键单击**。不支持**光标位置，横(纵)坐标偏移,横(纵)坐标百分比**的设置
<br> 当指定一个浏览器元素时。**点击方式** 属性选择 **设置控件**后，仅支持所选择元素所支持的JS事件；当有场景无法覆盖时，可更改属性值为**模拟鼠标**</td></table>
